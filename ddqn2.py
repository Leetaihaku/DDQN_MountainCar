...
def reward_calculator(state, next_state):
    return 2 * (abs(next_state + 0.55) - 0.65)
    '''pivot = 2*(abs(state + 0.55) - 0.65)
    if state > -0.55:
        pivot += 0.5 * pivot
    return pivot'''
    ...
    if answer == 'test':
        ...
        for E in range(EPISODES):
            ...
            for S in range(STEPS):
                ...
                action = agent.action_request(state)
                next_state, reward, done, _ = env.step(action)
                reward += reward_calculator(state[0], next_state[0])
